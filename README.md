...
- **逐次表示**: 文字起こしは確定した区間から順に画面へ流れます（文字起こし →
  話者識別 → 議事録生成の段階トラック付き）。
...
- **詳細画面**: 録音ごとに「文字起こし」「議事録」のタブを切り替えて閲覧できます。
  長い内容は画面右下の読み進みピルから、議事録の見出しや文字起こしの時間帯へ
  飛べます
- **PDF出力**: 詳細画面の「PDF出力」から、議事録と文字起こしをそれぞれ PDF で
  保存できます（日本語フォントは reportlab 内蔵の CID フォントを使うため、
  フォントファイルの追加インストールは不要です）
